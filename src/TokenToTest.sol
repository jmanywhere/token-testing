...
contract ManaCoin is Ownable, IERC20 {
    //@audit - SafeMath is irrelevant as of 0.8.0
    ...
    bool public isLimit; //@audit what does this do?
    ...
    // @audit These would be better off in a Struct
    ...
    // @audit Events should be descriptive of what happened
    ...
    constructor() {
        ...
        _symbol = "MNC";
        _decimals = 18;
        // @audit if decimals are to be used, to make this more readable you can use the keyword `ether`
        _totalSupply = 100_000_000 * (10 ** _decimals);
        // @audit - Initial minting, so should be OK to simply set the balance.
        _balances[owner()] = _balances[owner()].add(_totalSupply);

        treasuryWallet = payable(0x0aDEAE6683eFB0408542350E89B7B8311C4b6CE2);
        sellTax = 20;
        buyTax = 15;
        // @audit make these more readable...
        // e.g. could've used _totalSupply * 2 / 100;
        maxTxLimit = 2_000_000_000000000000000000;
        maxWalletLimit = 2_000_000_000000000000000000;
        ...
        // @audit Keep event emissions close to the action that should trigger them
    ...
    function removeReflectionExcluded(address account) external onlyOwner {
        require(
            isReflectionExcluded(account),
            "ERC20: Account must be excluded"
        );
    ...
    function _addReflectionExcluded(address account) internal {
        require(
            !isReflectionExcluded(account),
            "ERC20: Account must not be excluded"
        );
    ...
    function claimReflection() external returns (bool) {
        ...
        require(
            lastReflectionTimeStamp[_sender] + reflectionLockPeriod <=
                block.timestamp,
            "ERC20: Reflection lock period exists,  try again later"
        );
